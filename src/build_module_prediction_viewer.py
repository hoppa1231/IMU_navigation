...
<header><strong>GNSS-free route-holdout rollout</strong><select id="pick"></select></header><main><svg id="plot"></svg><aside><div class="k">Steps: <b id="steps"></b></div><div class="k">Mean horizontal error: <b id="mean"></b></div><div class="k">Final horizontal error: <b id="final"></b></div><div class="k">Max horizontal error: <b id="max"></b></div><p><span class="line" style="background:#2563eb"></span>GPS target trajectory</p><p><span class="line" style="background:#dc2626"></span>Accumulated model prediction</p><small>Each point is a non-overlapping prediction window. Equal X/Y scale; GPS is target/reference only.</small></aside></main>
<script>const cases={payload},pick=document.querySelector('#pick'),svg=document.querySelector('#plot');cases.forEach((c,i)=>pick.add(new Option(c.label,i)));function draw(){{const c=cases[pick.value||0],p=c.points,x=p.flatMap(q=>[q.te,q.pe]),y=p.flatMap(q=>[q.tn,q.pn]),r=svg.getBoundingClientRect(),pad=30,minx=Math.min(...x),maxx=Math.max(...x),miny=Math.min(...y),maxy=Math.max(...y),span=Math.max((maxx-minx)/(r.width-2*pad),(maxy-miny)/(r.height-2*pad),1),cx=(minx+maxx)/2,cy=(miny+maxy)/2,sx=v=>r.width/2+(v-cx)/span,sy=v=>r.height/2-(v-cy)/span,path=(a,b)=>p.map((q,i)=>`${{i?'L':'M'}}${{sx(q[a]).toFixed(1)}},${{sy(q[b]).toFixed(1)}}`).join('');svg.setAttribute('viewBox',`0 0 ${{r.width}} ${{r.height}}`);svg.innerHTML=`<path d="${{path('te','tn')}}" fill="none" stroke="#2563eb" stroke-width="2.5"/><path d="${{path('pe','pn')}}" fill="none" stroke="#dc2626" stroke-width="2.5"/>`;steps.textContent=c.steps;mean.textContent=c.meanError.toFixed(2)+' m';final.textContent=c.finalError.toFixed(2)+' m';max.textContent=c.maxError.toFixed(2)+' m'}}pick.onchange=draw;onresize=draw;draw()</script>'''
